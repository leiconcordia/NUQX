...
class _TrackerScreen extends State<TrackerScreen> {
  ...
  Future<void> checkForTransferNotification() async {
    if (queueInfo == null || queueInfo?['generatedQueuenumber'] == null) {
      print("⚠️ queueInfo or generatedQueuenumber is null. Skipping check.");
      setState(() {
    ...
    final queueNumber = queueInfo!['generatedQueuenumber'];

    // ✅ 1. Check if there is a transfer in queueNumbersCollection
    final latestTransfer = await MongoDatabase.getLatestTransferRecord(queueNumber);

    if (latestTransfer != null) {
      print("✅ Transfer record found for $queueNumber: $latestTransfer");

      // ✅ 2. Fetch related transfer notification from notificationsCollection
      final latestNotification =
      await MongoDatabase.getLatestTransferNotification(queueNumber);

      setState(() {
        hasBeenTransferred = true;
        transferMessage = latestNotification ?? 'You have been transferred';
      });
    } else {
      print("ℹ️ No transfer found for queueNumber: $queueNumber");
      setState(() {
        hasBeenTransferred = false;
        transferMessage = null;
      });
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (hasBeenTransferred && transferMessage != null)
            Text(
              transferMessage!,
              style: TextStyle(
                fontSize: 16.sp,
